...
# Import tất cả từ FindLaneLines.py
from ultils.FindLaneLines import *
...
@app.route('/')  
def upload():
	return render_template(template_homepage)  
...
def result():
	if request.method == 'POST':
		...
		if is_image:
			...
			final_text = ''
			return render_template(template_result,type_file = 'image', name=final_text, img_in=full_filename, img=full_filename_processed)
		elif is_video:
			# Xử lý video
			method_cv = FindLaneLines()
			i = randint(1, 1000000)
			...
			final_text = ''
			return render_template(template_result, type_file = 'video', name=final_text, video_in=full_filename, video=output_path)
